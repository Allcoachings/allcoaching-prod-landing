...
# 1) URLs already present in the Blog blogPost[] JSON-LD
...
# 2) all real posts on disk -> url, datePublished, folder, lang
def date_of(fp):
    s = open(fp, encoding='utf-8').read()
    mm = re.search(r'"datePublished"\s*:\s*"(\d{4}-\d{2}-\d{2})"', s)
    return mm.group(1) if mm else '0000-00-00'
posts = []
for folder in ['blog', 'blogs/en', 'blogs/hinglish']:
    for fp in sorted(glob.glob(folder + '/*.html')):
        ...
        posts.append({'folder': folder, 'slug': slug, 'urlpath': urlpath,
                      ...
                      'lang': 'hi-Latn' if folder == 'blogs/hinglish' else 'en-IN'})
print('total posts on disk:', len(posts))

# 3) the ones missing from the index, newest-first
...
# 4) harvest each missing post's card from its per-folder index
SRC = {'blog': 'blog/index.html', 'blogs/en': 'blogs/en/index.html', 'blogs/hinglish': 'blogs/hinglish/index.html'}
...
for p in missing:
    ...
    cards.append('    ' + card)
    bp_entries.append('{"@type":"BlogPosting","headline":"%s","url":"%s","datePublished":"%s","inLanguage":"%s"}'
                      % (headline, p['url'], p['date'], p['lang']))

# 5) prepend cards to the visible grid
